...
class Slash:
    ...
    def __init__(
        self, client, resolve_options=False, delete_unused=True, wait_sync=1
    ) -> None:
                        ...
                        for u in resolved["users"]:
                            ...
                            for op in fixed_options:
                                if options[op["name"]] == u:
                                    options[op["name"]] = discord.User(
                                        state=self._discord._get_state(),
                                        data=resolved_user,
                                    )
                        ...
                        for m in resolved["members"]:
                            ...
                            for op in fixed_options:
                                if options[op["name"]] == m:
                                    options[op["name"]] = discord.Member(
                                        state=self._discord._get_state(),
                                        data=resolved_member,
                                        guild=self._discord.get_guild(data["guild_id"]),
                                    )
            else:
                for op in fixed_options:
                    if op["type"] == OptionTypes.USER:
                        options[op["name"]] = await (
                            await self._discord.fetch_guild(data["guild_id"])
                        ).fetch_member(op["value"])
                    elif op["type"] == OptionTypes.CHANNEL:
                        options[op["name"]] = await self._discord.fetch_channel(
                            op["value"]
                        )
        ...
        for base_name in self.subcommand_groups:
            ...
            for x in subcommand_groups:
                ...
                for _base in x.base_names:
                    subs.append(
                        SlashOption(OptionTypes.SUB_COMMAND_GROUP, _base).to_dict()
                    )
    ...
    def subcommand_group(
        ...
        def wrapper(callback):
            ...
            self.subcommand_groups[base_names[0]][name] = SubSlashCommandGroup(
                ...
                guild_ids=guild_ids,
                default_permission=default_permission,
                guild_permissions=guild_permission,
            )
...
class Components:
    ...
    async def _on_socket_response(self, msg):
        ...
        msg = await getResponseMessage(
            self._discord, data=data, user=user, response=True
        )
        component = (
            EphemeralComponent(self._discord, user, data)
