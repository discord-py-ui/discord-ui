...
def getResponseMessage(state, data, user=None, response=True, bot_id=None):
    ...
    if response and user:
        return ResponseMessage(
            state=state, channel=channel, data=data, user=user, bot_id=bot_id
        )
...
class ResponseMessage(Message):
    ...
    async def defer(self, hidden=False):
        ...
        await self._state.http.request(
            V8Route(
                "POST",
                f'/interactions/{self.interaction_component.interaction["id"]}/{self.interaction_component.interaction["token"]}/callback',
            ),
            json=body,
        )
        self.deferred = True
        ...
        *,
        ...
        if not hidden:
            responseMSG = await self._state.http.request(
                V8Route(
                    "GET",
                    f"/webhooks/{self._bot_id}/{self.interaction_component.interaction['token']}/messages/@original",
                )
            )

            return getResponseMessage(
                self._state, responseMSG, response=False, bot_id=self._bot_id
            )
