...
@extension.slash.slashcommand(
    name="role-picker",
    description="let's you pick roles",
    guild_ids=["785567635802816595"],
)
async def command(ctx: SlashedCommand):

    # The role picker component
    role_picker = SelectMenu(
        "role_picker",
        options=[
            SelectOption(
                "javascript", "867715564155568158", "I'm a javascript programmer"
            ),
    ...
    try:
        ...
        menu, _ = await client.wait_for(
            "menu_select",
            lambda sel, msg: sel.custom_id == role_picker.custom_id
            and sel.member.id == ctx.member.id,
            timeout=20,
        )
        ...
        await menu.send(
            "I gave you following roles: `" + ", `".join(given_roles) + "`", hidden=True
        )
