...
@extension.slash.subcommand_group(
    base_names=["generate", "link"],
    name="button",
    description="sends a button and a linkbutton",
    options=[
...
async def command(
    ctx,
    message_content="cool, right?",
    name="click me",
    link="https://github.com/KusoRedsto/discord-message-components",
    emoji=None,
):
    ...
    if not link.startswith("http://") and not link.startswith("https://"):
        ...
        return await ctx.respond(
            "The link has to start with `http://` or `https://`", hidden=True
        )

    # Send a link button with the link, name and emoji argument
    await ctx.respond(
        content=message_content, components=[LinkButton(link, label=name, emoji=emoji)]
    )
