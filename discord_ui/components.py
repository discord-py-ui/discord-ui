...
class SelectOption:
    ...
    def __init__(
        self, value, label="\u200b", description=MISSING, emoji=MISSING
    ) -> None:
        """
    ...
    def label(self, value: str):
        if value is None:
            ...
            raise InvalidArgument(
                "label must be 100 or fewer and must be higher than 0 in length ("
                + str(len(value))
                + ")"
            )
    ...
    def description(self, value):
        if value is not MISSING and type(value) is not str:
            raise InvalidArgument(
                "description must be of type str, not " + " " + str(type(value))
            )
        if value is not MISSING and len(value) > 50:
            raise InvalidArgument(
                "description must be 50 or fewer in length (" + str(len(value)) + ")"
            )
...
class SelectMenu:
    ...
    def set_default_option(self, position: int) -> "SelectMenu":
        ...
        if type(position) is not int:
            raise InvalidArgument(
                "position has to be of type int, not " + str(type(position))
            )
...
class LinkButton:
    ...
    def __init__(
        self, url, label="\u200b", emoji=MISSING, new_line=False, disabled=False
    ) -> None:
        """
