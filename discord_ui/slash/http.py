...
async def get_command(command_name, client: com.Bot, guild_id=MISSING):
    return get(
        ...
        else (await get_guild_commands(client, guild_id)),
        command_name,
        lambda x: x.get("name"),
    )
...
async def delete_global_commands(client: com.Bot):
    commands = await client.http.request(
        BetterRoute("GET", f"/applications/{client.user.id}/commands")
    )
    for x in commands:
...
async def delete_guild_commands(client: com.Bot, guild_id):
    try:
        commands = await client.http.request(
            BetterRoute(
                "GET", f"/applications/{client.user.id}/guilds/{guild_id}/commands"
            )
        )
        for x in commands:
...
async def delete_global_command(client: com.Bot, command_id):
    return await client.http.request(
        BetterRoute("DELETE", f"/applications/{client.user.id}/commands/{command_id}")
    )
...
async def delete_guild_command(client: com.Bot, command_id, guild_id):
    return await client.http.request(
        BetterRoute(
            "DELETE",
            f"/applications/{client.user.id}/guilds/{guild_id}/commands/{command_id}",
        )
    )
...
async def get_command_permissions(client: com.Bot, command_id, guild_id):
    try:
        return await client.http.request(
            BetterRoute(
                "GET",
                f"/applications/{client.user.id}/guilds/{guild_id}/commands/{command_id}/permissions",
            )
        )
    except NotFound:
...
async def update_command_permissions(
    application_id, token, guild_id, command_id, permissions
):
    async with aiohttp.ClientSession() as client:
        async with client.put(
            f"https://discord.com/api/v9/applications/{application_id}/guilds/{guild_id}/commands/{command_id}/permissions",
...
async def create_global_command(command: dict, client: com.Bot):
    return await client.http.request(
        BetterRoute("POST", f"/applications/{client.user.id}/commands"), json=command
    )
...
async def create_guild_command(command, client: com.Bot, guild_id, permissions=[]):
    data = await client.http.request(
        BetterRoute(
            "POST", f"/applications/{client.user.id}/guilds/{guild_id}/commands"
        ),
        json=command,
    )
    return await update_command_permissions(
        client.user.id, client.http.token, guild_id, data["id"], permissions
    )


async def edit_global_command(command_id: str, client: com.Bot, new_command: dict):
    return await client.http.request(
        BetterRoute("PATCH", f"/applications/{client.user.id}/commands/{command_id}"),
        json=new_command,
    )


async def edit_guild_command(
    command_id, client: com.Bot, guild_id: str, new_command: dict, permissions: dict
):
    data = await client.http.request(
        BetterRoute(
            "PATCH",
...
async def get_guild_commands(client, guild_id):
    return await client.http.request(
        BetterRoute("GET", f"/applications/{client.user.id}/guilds/{guild_id}/commands")
    )
