        ...
        if x == "members":
            ...
            for message_id in data["data"]["resolved"]["messages"]:
                ...
                resolved["messages"][message_id] = discord.Message(
                    data=message_data,
                    channel=_discord.get_channel(data["channel_id"]),
                    state=_discord._connection,
                )
        elif x == "channels":
            resolved["channels"] = {}
            for channel_id in data["data"]["resolved"]["channels"]:
                ...
                if ChannelType(channel_data["type"]) is ChannelType.text:
                    channel = discord.TextChannel(
                        data=channel_data, guild=guild, state=_discord._connection
                    )
...
async def handle_options(data, options, method, _discord: discord.Client):
    _options = {}
    for op in options:
        _options[op["name"]] = await handle_thing(
            op["value"], op["type"], data, _discord, method
        )
    return _options
