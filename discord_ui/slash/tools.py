    ...
    for x in data["data"]["resolved"]:
        if x == "members":
            ...
            for m_id in data["data"]["resolved"]["members"]:
                ...
                resolved["members"][m_id] = discord.Member(
                    data=member_data,
            ...
            for message_id in data["data"]["resolved"]["messages"]:
                ...
                resolved["messages"][message_id] = discord.Message(
                    data=message_data,
                    channel=_state._get_channel(data["channel_id"]),
                    state=_state,
                )
        elif x == "channels":
            resolved["channels"] = {}
            for channel_id in data["data"]["resolved"]["channels"]:
                ...
                if ChannelType(channel_data["type"]) is ChannelType.text:
                    channel = discord.TextChannel(
                        data=channel_data, guild=guild, state=_state
                    )
            ...
            for role_id in data["data"]["resolved"]["roles"]:
                ...
                resolved["roles"][role_id] = discord.Role(
                    data=role_data,
                    guild=_state._get_guild(data["guild_id"]),
                    state=_state,
                )
...
async def fetch_data(value, typ, data, _discord):
    ...
    if typ == OptionType.MEMBER:
        return await (await _discord.fetch_guild(int(data["guild_id"]))).fetch_member(
            int(value)
        )
        ...
        return get(
            await (await _discord.fetch_guild(int(data["guild_id"]))).fetch_roles(),
            value,
            lambda x: getattr(x, "id", None) == int(value),
        )
    elif typ == AdditionalType.MESSAGE:
        return await (await _discord.fetch_channel(data["channel_id"])).fetch_message(
            int(value)
        )
    else:
        return value
...
def cache_data(value, typ, data, _state):
    logging.debug(
        "getting something out of the cache with type "
        + str(typ)
        + " value "
        + str(value)
    )
...
async def handle_thing(
    value, typ, data, method, _discord, auto=False
) -> typing.Union[
    str,
    int,
    bool,
    ...
    if method is ParseMethod.RESOLVE or method is ParseMethod.AUTO:
        ...
        try:
            ...
            logging.warning(
                "Got exepction while getting data from cache"
                + f"\n{type(ex).__name__}: {ex}\n"
                + f"{__file__}:{ex.__traceback__.tb_lineno}"
                + ("\nTrying next method" if method is ParseMethod.AUTO else "")
            )
            if auto is True:
                return await handle_thing(
                    value, typ, data, ParseMethod.CACHE, _discord, auto
                )
        ...
        try:
            ...
            if auto:
                return await handle_thing(
                    value, typ, data, ParseMethod.RAW, _discord, auto
                )
