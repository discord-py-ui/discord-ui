        ...
        if len(data["components"]) > 1:
            for componentWrapper in data["components"]:
                for btn in componentWrapper["components"]:
                    self.buttons.append(
                        Button._fromData(btn)
                        if not "url" in btn
                        else LinkButton._fromData(btn)
                    )
            ...
            self.buttons.append(
                ...
                if not "url" in data["components"][0]["components"][0]
    ...
    def respond(
        ...
        *,
        ...
        if ninjaMode:
            ...
            json = jsonifyMessage(
                content=content, embed=embed, file=file, files=files, buttons=buttons
            )
            if "embed" in json:
